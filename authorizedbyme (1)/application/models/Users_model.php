...
class Users_model extends My_Model {
    ...
    function getmessage($con) {
        $this->db->select('chat.*,CONCAT(users.firstname," ",users.lastname) as full_name,users.profilePic,CONCAT(to_user.firstname," ",to_user.lastname) as to_fullname,to_user.profilePic as to_profile,chat.attachment');
        $this->db->from('chat');
        $this->db->join('users','users.userId=chat.userfrom_id');
        $this->db->join('users to_user','to_user.userId=chat.userto_id');
        $this->db->where($con);
    ...
    function workers_fetchdata($limit, $start, $title, $search_location, $specialist, $userType, $usersubType) {
        ...
        if(!empty($data->result_array())) {
            foreach($data->result_array() as $row) {
                ...
                $output .= '<div class="emply-resume-list"> <div class="emply-resume-thumb">'.$profile_pic.'</div> <div class="emply-resume-info"> <h3><a href="#" title="">'.$name.'</a></h3><p><i class="la la-map-marker"></i>'. $row['address'].'</p> <p>'.$desc.'</p><p>&nbsp</p></div> <div class="shortlists" style="width:50px;"> <a href="'.base_url('page/worker-detail/'.base64_encode($row['userId'])).'" title="">View Profile<i class="la la-plus"></i></a> </div> </div>';
            }
        } else {
            $output .= '<div class="emply-resume-list"><div class="emply-resume-thumb"><h2>No Data Found</h2></div></div>';
        }
