 ...
 function fetch_data($limit, $start, $userId, $keyword, $sorting)
 {
  ...
  return $output;
	...
	function getChat($fromId,$toId) {
		// $query = $this->db->query('SELECT `chat`.*, CONCAT(users.firstname, " ", users.lastname) as full_name, `users`.`profilePic`, `to_user`.`username` as `to_username`, CONCAT(to_user.firstname, " ", to_user.lastname) as to_fullname FROM `chat` JOIN `users` ON `users`.`userId`=`chat`.`userfrom_id` JOIN `users` `to_user` ON `to_user`.`userId`=`chat`.`userto_id` where userfrom_id IN ("'.$fromId.'","'.$toId.'") and userto_id IN ("'.$fromId.'","'.$toId.'")');
		// echo $this->db->getCompiledSelect();die;
		// return $query->result();
		$query = $this->db->query('SELECT `chat`.*, CONCAT(users.firstname, " ", users.lastname) as full_name, `users`.`profilePic`,  CONCAT(to_user.firstname, " ", to_user.lastname) as to_fullname FROM `chat` JOIN `users` ON `users`.`userId`=`chat`.`userfrom_id` JOIN `users` `to_user` ON `to_user`.`userId`=`chat`.`userto_id` where userfrom_id IN ("'.$fromId.'","'.$toId.'") and userto_id IN ("'.$fromId.'","'.$toId.'")');
// echo $this->db->last_query(); // Display the raw SQL query
