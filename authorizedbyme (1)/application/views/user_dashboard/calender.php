              ...
              <div class="cardak">
                <div class="row">
                  <div class="col-xs-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    ...
                    <div class="col-md-6 col-6" style="display: inline-block; float: left;">
                      <?php
                      $availability = $this->db->query("SELECT * FROM user_availability WHERE user_id = '" . $_SESSION['authorized']['userId'] . "'")->result_array();
                      if (!empty($availability)) { ?>
                        <div class="job-overview" style="height: 382px; overflow: auto; margin-top: 0px;">
                          <p
                            style="width: 20%; display: inline-block; float: left; text-align: center; color: #000; font-size: 15px; font-weight: 600; font-family: Open Sans; margin: 0px !important;">
                            Start Date</p>
                          <p
                            style="width: 20%; display: inline-block; float: left; text-align: center; color: #000; font-size: 15px; font-weight: 600; font-family: Open Sans; margin: 0px !important;">
                            From Time</p>
                          ...
                          foreach ($availability as $value) { ?>
                            <div
                              style="width:80%; background: #c7c7c7; margin: 5px 0 0 0 !important; padding: 0; border-radius: 10px; box-shadow: 0 0 10px #dddddd; cursor: pointer;"
                              class="job-overview job_overview_main">
                              <p
                                style="width: 25%; display: inline-block; float: left; text-align: center; color: #000; font-size: 15px; font-weight: 600; font-family: Open Sans; margin: 0px !important;">
                            ...
                            <div>
                              <p style="width: 10%; display: inline-block; float: left; text-align: center; color: #000; font-size: 15px; font-weight: 600; font-family: Open Sans; margin: 0px !important;cursor: pointer;"
                                onclick="editAvailability('<?= $value['id'] ?>')"><i class="fa fa-edit"></i></p>
                              <p style="width: 10%; display: inline-block; float: left; text-align: center; color: #000; font-size: 15px; font-weight: 600; font-family: Open Sans; margin: 0px !important;cursor: pointer;"
                                onclick="deleteAvailability('<?= $value['id'] ?>')"><i class="fa fa-trash"></i></p>
                            </div>
                            <?php $i++;
                          } ?>
                        </div>
              ...
              <tbody id="clonetable_feedback1">
                <tr>
                  ...
                  <td><input type="hidden" name="user_id" id="user_id"
                      value="<?php echo @$_SESSION['authorized']['userId'] ?>"></td>
...
<script>
  $('#submit-button').on('click', function () {
    ...
    // Validation: Check if start and end dates are the same
    $("input[name=startdate]").each(function (index) {
      var startDate = $(this).val();
      var endDate = $("input[name=enddate]").eq(index).val();
      if (startDate !== endDate) {
        $.alert({
          title: '',
          content: "Start and End dates must be the same.",
        });
        return false; // Exit the loop if validation fails
      }
      sdateArray.push(startDate);
      edateArray.push(endDate);
    });

    // If validation failed, stop further processing
    if (sdateArray.length !== $("input[name=startdate]").length) {
      return false;
    }
    ...
    // form_data.append('start_date', JSON.stringify(sdateArray));
    // form_data.append('from_time', JSON.stringify(ftimeArray));
    // form_data.append('end_date', JSON.stringify(edateArray));
    // form_data.append('to_time', JSON.stringify(totimeArray));
